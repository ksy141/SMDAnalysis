...
class PackingDefectPMDA(ParallelAnalysisBase):
    ...
    def _single_frame(self, ts, atomgroups):
        ...
        if len(atomgroups) == 1:
            return Mup, Mdw, zlimup, zlimdw, ts.dimensions
        ...
        new_TRIO = TRIO.select_atoms('prop z>%f or prop z<%f'.format(utz-2, ltz+2))
        for atom in TRIO:
            xatom, yatom, zatom = atom.position
        ...
        bup = (trio_pos[:,2] > utz - 2)
        bdw = (trio_pos[:,2] < ltz + 2)
    ...
    def _conclude(self):
        ...
        Mup    = results[:,0]
        Mdw    = results[:,1]
        zlimup = np.vstack(results[:,2])
        zlimdw = np.vstack(results[:,3])
        dim    = np.vstack(results[:,4])

        Xmax   = max(dim[:,0])
        Ymax   = max(dim[:,1])
        
        xarray = np.arange(0, Xmax, self.dx)
        yarray = np.arange(0, Ymax, self.dy)
        xx, yy = np.meshgrid(xarray, yarray)

        N = len(xarray) * len(yarray) * 2
        ...
        with mda.Writer('defect.pdb', multiframe=True, n_atoms=N) as PDB:
            for ts in df.trajectory:
                num = 0
                
                ## PL acyl -> tempfactor = 1
                bA = (1e3 <= Mup[i]) & (Mup[i] < 1e6)
                for x1, y1 in zip(xx[bA], yy[bA]):
                    df.atoms[num].position = np.array([y1, x1, zlimup])
                    df.atoms[num].tempfactor = 1
                    num += 1
                
                bA = (1e3 <= Mdw[i]) & (Mdw[i] < 1e6)
                for x1, y1 in zip(xx[bA], yy[bA]):
                    df.atoms[num].position = np.array([y1, x1, zlimdw])
                    df.atoms[num].tempfactor = 1
                    num += 1
