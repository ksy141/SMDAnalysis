#!/usr/bin/env python
"""
charmmgui2dms.py: convert a charmm-gui system to a Desmond dms/cms, LAMMPS, or Tinker file
...
class atom:
    ...
    def __init__(self, atomid, segid, resid, resname, atomname, atomtype, charge, mass):
        ...
        self.atomstr = "-".join([self.segid, str(self.resid),\
                self.resname, self.atomname, self.atomtype, str(self.charge)])
        self.anum = self.get_anum(self.mass)
...
class psf:
    ...
    def get_uniq_bond_pairs(self):
        bond_pairs = []
        for i in self.bond:
            types = [ self.atom[j-1].atomtype for j in i ]
            type1 = " ".join(types)
            if not type1 in bond_pairs:
                bond_pairs.append(type1)
        return bond_pairs

    def get_uniq_angle_pairs(self):
        angle_pairs = []
        for i in self.angle:
            types = [ self.atom[j-1].atomtype for j in i ]
            type1 = " ".join(types)
            if not type1 in angle_pairs:
                angle_pairs.append(type1)
        return angle_pairs
    ...
    def get_uniq_cmap_pairs(self):
        cmap_pairs = []
        for i in self.cmap:
            types = [ self.atom[j-1].atomtype for j in i ]
            type1 = " ".join(types)
            if not type1 in cmap_pairs:
                cmap_pairs.append(type1)
        return cmap_pairs
...
class crd:
    ...
    def read_crd(self, crdfile):
        ...
        print("read",len(self.atom), "atoms from", crdfile)
...
class prm_bond:
    ...
    def __eq__(self, other):
        if self.name == other.name or self.name == other.name[::-1]:
            ...
            return False
    def get_par(self):
        return [self.kb, self.b0, self.memo]
...
class prm_angle:
    ...
    def __eq__(self, other):
        if self.atom == other.atom or self.atom == other.atom[::-1]:
            ...
            return False
    def get_par(self):
        return [self.Ktheta, self.Theta0, self.Kub, self.S0, self.memo]
...
class prm_dihe:
    ...
    def get_par(self):
        return [self.dihe_orig, self.memo]
    ...
    def add_mult(self, names, kchi, n, delta):
        ...
        if [n, kchi, delta] in self.dihe_orig:
            print("WARNING duplicate dihe term %s, will skip"%(" ".join(self.names)), self.dihe_orig)
        ...
        for i in self.dihe_orig:
            ...
            if n == i[0]:
                ...
                print("WARNING overwriting dihe term %s, multi %d, chi %f delta %f -> chi %f delta %f"%(self.namestr, n,\
                        k, d, kchi, delta))
        if not tag:
...
class prm_impr:
    def __init__(self, atoms, kpsi, psi0, memo = ""):
        ...
        self.memo = memo.replace("'", " ")
    def __eq__(self, other):
        if (self.atoms == other.atoms[::-1] or self.atoms == other.atoms)\
                and self.kpsi - other.kpsi < 1e-4 \
                and self.psi0 - other.psi0 < 1e-4:
            return True
        else:
            return False
    def get_par(self):
        return [self.kpsi, self.psi0, self.memo]

class prm_nonbond:
    def __init__(self, name, epsilon, sigma, epsilon_14, sigma_14, rmin, rmin_14, memo = ""):
        self.name = name
        ...
        #original rmin and rmin_14 from charmm FF
        self.rmin = rmin
        self.rmin_14 = rmin_14
        self.memo = memo.replace("'", " ")

    def get_par(self):
        return [self.epsilon, self.sigma, self.epsilon_14, self.sigma_14, self.rmin, self.rmin_14, self.memo]
...
class prm_cmap:
    ...
    def get_par(self):
        return [self.ntorsion, self.phi, self.psi, self.cmap]
...
class prm:
    ...
    def get_atom_memo(self, type):
        id = self.get_mass_par_index(type)
        return self.atom[id].memo

    def get_mass(self, type):
        atomid = self.get_mass_par_index(type)
        return self.atom[atomid].mass

    def get_nonbondpar(self, type):
        id = self.get_nonbond_par_index(type)
        return self.nonbond[id].get_par()

    def get_bondpar(self, type1, type2):
        id = self.get_bond_par_index(type1, type2)
        return self.bond[id].get_par()

    def get_anglepar(self, type1, type2, type3):
        id = self.get_angle_par_index(type1, type2, type3)
        return self.angle[id].get_par()

    def get_imprpar(self, type1, type2, type3, type4):
        id = self.get_impr_par_index(type1, type2, type3, type4)
        return self.impr[id].get_par()
    
    def get_dihepar(self, type1, type2, type3, type4):
        id = self.get_dihe_par_index(type1, type2, type3, type4)
        red = []
        for i in id:
            red = red + self.dihe[i].get_par()
        return red

    def get_cmappar(self, typelist):
        id = self.get_cmap_par_index(typelist)
        return self.cmap[id].get_par()
    ...
    def add_impr(self,linee):
        ...
        if imp in self.impr:
            print("duplicated impr para", linee, "skip")
            return 0
    ...
    def add_nonbond(self,linee):
        ...
        nb = prm_nonbond(name, epsilon, sigma, epsilon_14, sigma_14, rmin/2.0, rmin_14/2.0, memo)
...
class cms:
    ...
    def generate_cms(self, psf, crd, prm, temperature, boxsize, posref, atom_velocity):
        ...
        ma = """i_m_mmod_type r_m_x_coord r_m_y_coord r_m_z_coord i_m_residue_number s_m_insertion_code
        ...
        for i in range(0, psf.atomnum):
            ...
            if insertion == " ":
                ...
                term = "%d %f %f %f %d %s %d %s %s %s %d %f %s %f %f %f"%(mmodtype, x, y, z, resi, insertion, color, chainid, resname,\
                    name, anum, formal_charge, name, vx, vy, vz)
...
class tinker:
    ...
    def generate_tinker(self, inputpsf, inputcrd, charmmprm, boxsize):
        fpxyz = open(self.output + ".xyz", "w")

        fpxyz.write("%8d %s\n"%(inputpsf.atomnum, self.title))
        boxsize = [boxsize[0][0], boxsize[1][1], boxsize[2][2]]
        box = " ".join(["%16.8f"%i for i in boxsize + [90, 90, 90]])
        fpxyz.write("%s\n"%box)
        
        bonddict = {}
        for i in range(inputpsf.atomnum):
            bonddict[i+1] = []
        for i in range(inputpsf.bondnum):
            id1, id2 = inputpsf.bond[i]
            bonddict[id1].append(id2)
            bonddict[id2].append(id1)
        
        atomtypes = []
        atomtypes_dict = {}
        atomtypes_anum_dict = {}
        atomtypes_charge_dict = {}
        for i in range(inputpsf.atomnum):
            c = inputcrd.get_coor(i)
            atomtype = inputpsf.atom[i].get_atomtype()
            if not atomtype in atomtypes:
                atomtypes.append(atomtype)
                atomtypes_dict[atomtype] = len(atomtypes)
                atomtypes_charge_dict[atomtype] = inputpsf.atom[i].charge
            typeid = atomtypes_dict[atomtype]
            atomtypes_anum_dict[atomtype] = inputpsf.atom[i].anum
            fpxyz.write("%8d %6s %16.8f %16.8f %16.8f %4d "%(i+1, inputpsf.atom[i].atomname, c[0], c[1], c[2], typeid))
            for j in sorted(bonddict[i+1]):
                fpxyz.write("%8d "%j)
            fpxyz.write("\n")

        fppar = open(self.output + ".prm", "w")
        fppar.write("forcefield              CHARMM36\n")
        fppar.write("vdwtype                 LENNARD-JONES\n")
        fppar.write("radiusrule              ARITHMETIC\n")
        fppar.write("radiustype              R-MIN\n")
        fppar.write("radiussize              RADIUS\n")
        fppar.write("epsilonrule             GEOMETRIC\n")
        fppar.write("vdw-14-scale            1.0\n")
        fppar.write("chg-14-scale            1.0\n")
        fppar.write("electric                332.0716\n")
        fppar.write("dielectric              1.0\n")

        nbond = np.zeros(inputpsf.atomnum, dtype=np.int)
        nbond_atomtypes = [inputpsf.atom[i].get_atomtype() for i in range(inputpsf.atomnum)]
        for b in inputpsf.bond:
            nbond[b[0]-1] += 1
            nbond[b[1]-1] += 1
        nbond_count = list(set(zip(nbond_atomtypes, nbond)))
        nbond_count.sort(key=lambda x: x[0])
        nbond_dict = {}
        for i in nbond_count:
            if i[0] in nbond_count:
                print(nbond_count)
                print("atomtype %s has >1 types of bonded atoms")
            nbond_dict[i[0]] = i[1]

        for i in range(len(atomtypes)):
            at = atomtypes[i]
            mass = charmmprm.get_mass(at)
            anum = atomtypes_anum_dict[at]
            memo = charmmprm.get_atom_memo(at)
            if len(memo) > 20:
                memo = memo[0:20] + "..."
            memo = "\"" + memo + "\""
            nbonded = nbond_dict[at]
            fppar.write("atom   %8d %8d %5s   %-25s  %3d %8.3f %2d\n"%(i+1, i+1, at, memo, anum, mass, nbonded))

        for i in range(len(atomtypes)):
            fppar.write("charge   %8d   %8.4f\n"%(i+1, atomtypes_charge_dict[atomtypes[i]]))

        for i in range(len(atomtypes)):
            at = atomtypes[i]
            epsilon, sigma, epsilon_14, sigma_14, rmin, rmin_14, memo = charmmprm.get_nonbondpar(at)
            fppar.write("vdw    %8d %8.4f %8.4f\n"%(i+1, rmin, -epsilon))
            if sigma_14 > 0:
                fppar.write("vdw14  %8d %8.4f %8.4f\n"%(i+1, rmin_14, -epsilon))
        
        bondpairs = inputpsf.get_uniq_bond_pairs()
        for bp in bondpairs:
            bp = bp.split()
            typeid1 = atomtypes_dict[bp[0]]
            typeid2 = atomtypes_dict[bp[1]]
            kb, b0, memo = charmmprm.get_bondpar(bp[0], bp[1])
            fppar.write("bond  %5d %5d  %8.2f %8.4f\n"%(typeid1, typeid2, kb, b0))
 
        anglepairs = inputpsf.get_uniq_angle_pairs()
        for ag in anglepairs:
            ag = ag.split()
            typeid1 = atomtypes_dict[ag[0]]
            typeid2 = atomtypes_dict[ag[1]]
            typeid3 = atomtypes_dict[ag[2]]
            ktheta, theta0, kub, s0, memo = charmmprm.get_anglepar(ag[0], ag[1], ag[2])
            fppar.write("angle     %5d %5d %5d  %8.4f  %8.4f\n"%(typeid1, typeid2, typeid3, ktheta, theta0))
            if kub > 0:
                fppar.write("ureybrad  %5d %5d %5d  %8.4f  %8.4f\n"%(typeid1, typeid2, typeid3, kub, s0))

        imprpairs = inputpsf.get_uniq_impr_pairs()
        impr = []
        for i in imprpairs:
            ii = i.split()
            impr.append([ii, charmmprm.get_imprpar(ii[0], ii[1], ii[2], ii[3])] )
        nimpr = len(impr)
        distanceX = np.ones((nimpr, nimpr))
        for i in range(nimpr):
            distanceX[i][i] = 0
            for j in range(i+1, nimpr):
                if " ".join(impr[i][0]) == " ".join(impr[j][0]):
                    distanceX[i][j] = 0
                    distanceX[j][i] = 0
                if " ".join(impr[i][0]) == " ".join([impr[j][0][0], impr[j][0][2], impr[j][0][1], impr[j][0][3]]):
                    distanceX[i][j] = 0
                    distanceX[j][i] = 0
        sqfrm = squareform(distanceX)
        clustering = linkage(sqfrm, method='average')
        clusres = fcluster(clustering, t=0.5, criterion='distance')
        clusdict = {}
        for i in range(len(clusres)):
            if not clusres[i] in clusdict:
                clusdict[clusres[i]] = []
            clusdict[clusres[i]].append(i)
        for i in clusdict:
            imp = impr[clusdict[i][0]]
            typeid1 = atomtypes_dict[imp[0][0]]
            typeid2 = atomtypes_dict[imp[0][1]]
            typeid3 = atomtypes_dict[imp[0][2]]
            typeid4 = atomtypes_dict[imp[0][3]]
            psi = impr[i][1][1]
            f = 0.0 
            for j in clusdict[i]:
                f += impr[j][1][0]
            if len(clusdict[i]) > 1:
                k = [" ".join(impr[j][0]) + " %5.2f %5.2f"%(impr[j][1][0], impr[j][1][1]) for j in clusdict[i]]
                fppar.write("#merged from %s\n"%";".join(k))
            fppar.write("improper  %5d %5d %5d %5d  %6.2f  %6.2f\n"%(typeid1, typeid2, typeid3, typeid4, f, psi))

        dihepairs = inputpsf.get_uniq_dihedral_pairs()
        for dh in dihepairs:
            dh = dh.split()
            typeid1 = atomtypes_dict[dh[0]]
            typeid2 = atomtypes_dict[dh[1]]
            typeid3 = atomtypes_dict[dh[2]]
            typeid4 = atomtypes_dict[dh[3]]
            dhpar, memo = charmmprm.get_dihepar(dh[0], dh[1], dh[2], dh[3])
            fppar.write("torsion  %5d %5d %5d %5d"%(typeid1, typeid2, typeid3, typeid4))
            for d in dhpar:
                # 0.050  180.0  6
                fppar.write(" %6.3f  %6.1f  %2d "%(d[1], d[2], d[0]))
            fppar.write("\n")
            
        cmapairs = inputpsf.get_uniq_cmap_pairs()
        for cp in cmapairs:
            cp = cp.split()
            typeid1 = atomtypes_dict[cp[0]]
            typeid2 = atomtypes_dict[cp[1]]
            typeid3 = atomtypes_dict[cp[2]]
            typeid4 = atomtypes_dict[cp[3]]
            typeid5 = atomtypes_dict[cp[4]]
            ntorsion, phi, psi, cmapvalue = charmmprm.get_cmappar(cp)
            cmapvalue = np.array(cmapvalue)
            cmapvalue = cmapvalue.reshape((ntorsion, ntorsion))
            cmapvalue = np.pad(cmapvalue, ((0,1),(0,1)), mode='constant', constant_values=0)
            phi = np.append(phi, 180.0)
            psi = np.append(psi, 180.0)
            for i in range(ntorsion+1):
                cmapvalue[i][ntorsion] = copy.deepcopy(cmapvalue[i][0])
                cmapvalue[ntorsion][i] = copy.deepcopy(cmapvalue[0][i])
            cmapvalue = np.ndarray.flatten(cmapvalue)
            fppar.write("tortors  %5d  %5d  %5d  %5d  %5d  %3d  %3d\n"%(typeid1, typeid2, typeid3, typeid4,\
                    typeid5, ntorsion+1, ntorsion+1))
            count = 0
            for i in phi:
                for j in psi:
                    fppar.write("%8.1f  %8.1f  %10.5f\n"%(i, j, cmapvalue[count]))
                    count += 1
    ...
    def generate_lammps(self, psf, crd, prm, boxsize, lammpscmap, lammpsangle, zcenter):
        ...
        if psf.dihenum > 0:
            ...
            for i in range(0, psf.dihenum):
                ...
                if (id1, id2) in anglebondpair or (id2, id1) in anglebondpair:
                    ...
                    weight = 1.0/len(dihe_14atom_dict[(id1, id2)])

                """
                elif len(dihe_14atom_dict[(id1, id2)]) == 2:
                    weight = 0.5
                elif len(dihe_14atom_dict[(id1, id2)]) == 4:
                    weight = 0.25
                elif len(dihe_14atom_dict[(id1, id2)]) > 4 or len(dihe_14atom_dict[(id1, id2)]) == 3:
        ...
        if psf.anglenum > 0:
            ...
            for i in range(0, len(angletypes)):
                ...
                mass0 = prm.get_mass(prm.angle[angleid].atom[0])
                mass1 = prm.get_mass(prm.angle[angleid].atom[1])
                mass2 = prm.get_mass(prm.angle[angleid].atom[2])
                if int(mass0) == 1 and np.fabs(mass1-15.9994) <=1e-3 and int(mass2) == 1:
                    constrained_angle.append(str(i+1))
            if len(constrained_angle) > 0 and lammpsangle:
...
def main():
    ...
    parser.add_argument('-tinker', action="store", dest="outputtinker",  required=False,\
            help="output tinker xyz and key file")
    ...
    outputtinker = inputarg.outputtinker
    ...
    if outputtinker != None:
        tinkerfile = tinker(outputtinker)
        tinkerfile.generate_tinker(inputpsf, inputcrd, charmmprm, boxsize)
